...
(defun chatwork-insert-tag-to-contacts (contact)
  ...
  (let (contact-info account-id name)
    (setq contact-info (cdr (assoc contact `(,@chatwork-contacts-name-alist ,@chatwork-contacts-id-alist)))
          account-id (car contact-info)
          name (cdr contact-info))
